<template>
  <tbody v-if="mode=='AlarmLogDB'" class="text-sm font-medium divide-y divide-gray-100 dark:divide-gray-700/60">
    <tr v-for="(item, index) in eventData" :key="index" class="h-14">
      <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
        <div class="font-medium text-gray-800 dark:text-gray-100">{{ item.ts_formatted }}</div>
      </td>
      <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
        <div class="text-left font-medium text-green-600">{{ item.criteria }}</div>
      </td>
      <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
        <div class="text-left" :class="item.status === 'OCCURRED' ? 'text-red-500' : 'text-blue-500'">{{ item.status }}</div>
      </td>
      <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
  ...
  <tbody v-else-if="mode=='Event'" class="text-sm font-medium divide-y divide-gray-100 dark:divide-gray-700/60">
    <!-- Data rows -->
    <tr v-for="(item, index) in eventData" :key="index" class="h-14">
      <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
        <div>{{ item.Type }}</div>
