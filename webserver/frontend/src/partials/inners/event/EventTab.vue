...
export default {
  ...
  setup(props){
    ...
    const fetchAlarmLogDB = async (pg) => {
      try {
        const response = await axios.get(`/api/getAlarmLog/${pg}`);
        if(response.data.success){
          eventData.value = response.data.data;
          showPrev.value = response.data.hasPrev;
          showNext.value = response.data.hasNext;
        ...
        console.error("알람로그(DB) 가져오기 실패:", error);
      }
    };

    const searchAlarmLogDB = async (pg) => {
      const searchRequest = {
        StartDate: start.value,
        EndDate: end.value,
      };
      try {
        const response = await axios.post(`/api/getAlarmLogSearch/${pg}`, searchRequest, {
          headers: { "Content-Type": "application/json" },
        });
        if(response.data.success){
          eventData.value = response.data.data;
        ...
        console.error("알람로그(DB) 검색 실패:", error);
      }
    };
    ...
    onMounted(()=>{
      ...
      else if(mode.value == 'AlarmLogDB')
        fetchAlarmLogDB(curPage.value);
      else
    ...
    watch(() => curPage.value, (newPage) => {
      if(mode.value == 'AlarmLogDB'){
        if(!period.value){
          fetchAlarmLogDB(newPage);
        }else{
          searchAlarmLogDB(newPage);
        }
