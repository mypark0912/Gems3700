  ...
  setup(props) {
    ...
    const isCurrentChannelDiagnosisActive = computed(() => {
      if (channel.value === "Main") {
        return (
          General_inputDict.value.useFunction?.diagnosis_main === true ||
          General_inputDict.value.useFunction?.diagnosis_main === 1
        );
      } else if (channel.value === "Sub") {
        return (
          General_inputDict.value.useFunction?.diagnosis_sub === true ||
          General_inputDict.value.useFunction?.diagnosis_sub === 1
        );
      }
      return false;
    });
    ...
    const savefile = async (source = "default") => {
      ...
      const isFTPEnabled = General_inputDict.value.useFunction.ftp === 1;
