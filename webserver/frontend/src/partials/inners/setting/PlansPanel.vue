          ...
          <div class="flex items-center space-x-4 text-sm relative">
            ...
            <label v-if="false" class="flex items-center space-x-2">
              <input
                ...
                :checked="
                  inputDict.useFunction.ftp === 1 ||
                  inputDict.useFunction.ftp === true
                "
            ...
            <label v-if="false" class="flex items-center space-x-2">
              <input
                ...
                class="form-checkbox text-violet-500 focus:ring-violet-500"
                :checked="
                  inputDict.useFunction.sntp === 1 ||
                  inputDict.useFunction.sntp === true
                "
                @change="
                  inputDict.useFunction.sntp = $event.target.checked ? 1 : 0
                "
              />
...
export default {
  ...
  setup(props) {
    ...
    const handleFTPToggle = (event) => {
      ...
      inputDict.value.useFunction.ftp = isChecked ? 1 : 0;
    ...
    watch(
      () => inputDict.value.useFunction?.ftp,
      (newVal) => {
    ...
    const savefile = async () => {
      ...
      const isFTPEnabled = inputDict.value.useFunction.ftp === 1;

      // inputDict의 diagnosis 설정값을 사용 (Setting.vue에서 watch로 연동됨)
      const isMainDiagnosisEnabled =
        inputDict.value.useFunction.diagnosis_main === true ||
        inputDict.value.useFunction.diagnosis_main === 1;
      const isSubDiagnosisEnabled =
        inputDict.value.useFunction.diagnosis_sub === true ||
        inputDict.value.useFunction.diagnosis_sub === 1;
